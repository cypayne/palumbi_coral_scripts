...
for i in $FQ; do
    ...
    echo "srun bowtie2 -p 16 -X 1500 --rg-id $BASE --rg SM:$BASE -D 20 -R 3 -N 1 --score-min L,0,-1.8 -L 20 -i S,1,0.50  -x $1 -1 ${BASE}_1.txt.gz -2 ${BASE}_2.txt.gz > $BASE.sam" >> TEMPBATCH.sbatch
    echo "samtools view -bSq 10 ${BASE}.sam > ${BASE}_BTVS-UNSORTED.bam" >> TEMPBATCH.sbatch
    #echo "samtools view -bSq 10 ${BASE}.sam > ${BASE}_BTVS-UNSORTED" >> TEMPBATCH.sbatch
