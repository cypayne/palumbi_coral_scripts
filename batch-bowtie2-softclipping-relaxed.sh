...
for i in $FQ; do
    ...
    echo "srun bowtie2 --sensitive-local -p 8 -X 1500 --gbar 1 --mp 3 --rg-id $BASE --rg SM:$BASE -x $1 -1 ${BASE}_1.txt.gz -2 ${BASE}_2.txt.gz > $BASE.sam" >> SOFT-TEMPBATCH.sbatch
    ...
    echo "srun samtools index ${BASE}.bam" >> SOFT-TEMPBATCH.sbatch
    echo "rm ${BASE}.sam" >> SOFT-TEMPBATCH.sbatch
    echo "rm ${BASE}_BTVS-UNSORTED.bam" >> SOFT-TEMPBATCH.sbatch
    echo "rm ${BASE}_UNDEDUP.bam" >> SOFT-TEMPBATCH.sbatch
